package ${basepackage}.web.controller.api.v1;
...
/**
 * Api接口：[${table.className}]Controller   ${table.remarks!""}
 */
@RestController
@RequestMapping("/tg/api/v1/${table.classNameFirstLower}")
public class Api${table.className}Controller extends TgSpringBaseController {
    @Autowired
     ...
     */
    @RequestMapping(value = "/findEnabled")
     ...
     */
    @RequestMapping(value = "/getByCodeNum")
    public TgResponseBean getByCodeNum(@RequestParam("codeNum") final String codeNum) throws TgBusinessException {
        ${table.className} ${table.classNameFirstLower} = ${table.classNameFirstLower}Service.getByCodeNum(id);
        return new TgResponseBean(${table.classNameFirstLower});
    }
     ...
     * 停用[${table.className}]
     */
    @RequestMapping(value = "/disable.api")
